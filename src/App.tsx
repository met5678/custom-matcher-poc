...
const GRID_SEED = "emily-seed-002";
...
function compactGrid(grid: Cell[], size: number): Cell[] {
  ...
  const nonEmptyRows: Cell[][] = rows.filter((row) =>
...
function App() {
  ...
  useEffect(() => {
    return () => {
      flashTimeouts.current.forEach((timeoutId) =>
  ...
  function handleCellClick(index: number) {
    ...
    const secondCategoryId =
      "choices" in cell ? cell.categoryId : cell.categoryId;
  ...
  function handleReset() {
    ...
    const confirmed = window.confirm(
      "Reset the game? This will clear your progress.",
    );
    if (!confirmed) return;
    const grid = buildGrid(choices, GRID_COUNT, GRID_SEED);
